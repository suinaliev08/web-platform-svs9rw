...
let $watchStates = !0,
  $debug = !1;
const setWatchStates = function (t) {
  $watchStates = t;
};
$debug &&
...
const stateUpdateDepth = (t, e) => {
    const a = t.split('.');
    return a.length > 1 ? ($state[a[0]][a[1]] = e) : ($state[t] = e);
  },
  stateCheckDepth = (t, e) => {
    const a = t.split('.');
    return a.length > 1 ? $state[a[0]][a[1]] : $state[t];
  },
  setState = function (t, e) {
    const a = $state[t];
    stateUpdateDepth(t, e),
      setStateEvent(t, e, a),
      updateDom(t, e),
      watchExpressions();
  },
  ...
  updateDom = function (name, value) {
    let el;
    for (v in value) {
      $.isNumeric(v) ||
        ((el = $('[data-state="' + name + '.' + v + '"]')),
        (value = eval('$state.' + name + '.' + v))),
        el || (el = $('[data-state="' + name + '"]')),
        el.is('input:not(:checkbox):not(:radio):not(:file)') && el.val(value),
        el.prop('tagName') === 'TEXTAREA' && el.val(value),
        el.is(':checkbox') && el.prop('checked', value),
        el.is(':radio') && el.prop('checked', value),
        el.is(':file') && el.val(value),
        el.is('select') &&
          (el.attr('multiple') && typeof value == 'object'
            ? el.find('option').each(function () {
                (value.includes($(this).val()) ||
                  value.includes(parseFloat($(this).val()))) &&
              ...
              function (match, contents) {
                return (
                  (contents = contents
                    ...
                    .replaceAll('&gt;', '>')
                    .replaceAll('&lt;', '<')),
                  typeof eval(contents) == 'object'
                    ? JSON.stringify(eval(contents)).replaceAll('"', "'")
                    : eval(contents)
                );
              }
            )),
  ...
  watchStates = function () {
    $(document)
      .off('keyup keypress input', '[data-input]')
      .on('keyup keypress input', '[data-input]', function () {
        let t = $(this).data('state'),
          e = $(this).val();
        e.match(/^[0-9\.\,]+$/) && (e = parseFloat(e)),
          stateCheckDepth(t) !== e &&
            (setStateEvent(t, e, $state[t]),
            stateUpdateDepth(t, e),
            watchExpressions());
      }),
      $(document)
        .off('keyup keypress input', '[data-textarea]')
        .on('keyup keypress input', '[data-textarea]', function () {
          let t = $(this).data('state'),
            e = $(this).val();
          e.match(/^[0-9\.\,]+$/) && (e = parseFloat(e)),
            stateCheckDepth(t) !== e &&
      ...
      $(document)
        .off('change', '[data-checkbox]')
        .on('change', '[data-checkbox]', function () {
          const t = $(this).data('state'),
            e = $(this).prop('checked');
          stateCheckDepth(t) !== e &&
            (setStateEvent(t, e, $state[t]),
            stateUpdateDepth(t, e),
      ...
      $(document)
        .off('change', '[data-file]')
        .on('change', '[data-file]', function () {
          let t = $(this).data('state'),
            e = $(this)[0].files;
          $(this).attr('multiple') || (e = e[0]),
            stateCheckDepth(t) !== e &&
              (setStateEvent(t, e, $state[t]),
              stateUpdateDepth(t, e),
              watchExpressions());
      ...
      $(document)
        .off('change', '[data-select]')
        .on('change', '[data-select]', function () {
          const t = $(this).data('state'),
            e = $(this).val();
          typeof e != 'object' &&
            stateCheckDepth(t) !== e &&
  ...
  stateEffect = function (t, e = !1) {
    e
      ? $.each(e, (a, s) => {
          $(document).on(s, function (n, i, l) {
            t(i, l, s);
          });
        })
      : t();
        ...
        (html = html.replaceAll(/\{(.*?)\}/gsu, function (expression, content) {
          return (
            (content = content
              ...
              .replaceAll(/###(.*?)###/g, '${$1}')),
            content.match(/[\?\&\=\s\+\(\)]+/g)
              ? `<span data-expression="${content}">${eval(content)}</span>`
              : `<span data-text="${content.replace('$state.', '')}">${eval(
                  content
                )}</span>`
          );
        })),
  ...
  setDomStates = function () {
    $('[data-states]').each(function () {
      const states = eval(
        'JSON.parse(JSON.stringify(' + $(this).data('states') + '))'
      );
      for (state in states) $state[state] = states[state];
    }),
      $('[data-state]').each(function () {
        ...
        nameSplited.length > 1
          ? (typeof $state[nameSplited[0]] == 'undefined' &&
              ($state[nameSplited[0]] = {}),
            (_state = $state[nameSplited[0]][nameSplited[1]] = value))
          : (_state = $state[name]),
          $(this).is('input:not(:checkbox):not(:radio):not(:file)') &&
            ((value = $(this).val() || null),
            value && value.match(/^[0-9\.\,]+$/) && (value = parseFloat(value)),
            $(this).attr('data-input', ''),
            _state && $(this).val(_state)),
          $(this).is(':checkbox') &&
            ((value = $(this).prop('checked') || !1),
            _state && $(this).prop('checked', _state),
            $(this).attr('data-checkbox', '')),
          $(this).is(':radio') &&
            (value || (value = $(this).filter(':checked').val() || !1),
            _state && $(this).prop('checked', _state),
          ...
          $(this).prop('tagName') === 'TEXTAREA' &&
            ...
            _state && $(this).val(_state)),
          $(this).is(':file:not([data-custom])') &&
            ((value = !1), $(this).attr('data-file', '')),
          $(this).is('select') &&
            ($(this).attr('multiple')
              ? (value = $(this).val())
              : (value =
                  $('option:selected:not(:disabled)', this).val() || null),
            _state &&
  ...
  setStateEvent = function (t, e, a) {
    $(document).trigger(t, [e, a]);
  },
  getState = function (t) {
    return $state[t];
  },
...
dataApiInit(), $(document).trigger('jq-data-api');
